...
kernel_install:
	...
	cp $(kernel_addon_PATH)/fs-patches/infoadas-overlay-vision_2.9.0.0.tar.gz $(LINUX_TARGETFS)/
...
ifeq ($(PROC_IPU2_INCLUDE),yes)
	cp $(vision_sdk_PATH)/binaries/vision_sdk/bin/$(PLATFORM)/vision_sdk_ipu2_$(PROFILE_ipu2).xem4 $(LINUX_TARGETFS)/lib/firmware/dra7-ipu2-fw.xem4
...
ifeq ($(PROC_DSP2_INCLUDE),yes)
	...
	cp $(vision_sdk_PATH)/linux/scripts/hdmiReceiverConfig.sh $(LINUX_TARGETFS)/opt/vision_sdk
	cp $(vision_sdk_PATH)/linux/scripts/disableDssInterruptsOnA15.sh $(LINUX_TARGETFS)/opt/vision_sdk
	cp $(vision_sdk_PATH)/linux/scripts/enableIpu2CCSConectivity.sh $(LINUX_TARGETFS)/opt/vision_sdk
	...
	cp $(vision_sdk_PATH)/linux/src/system/sgxRenderUtils/jeep2.bmp $(LINUX_TARGETFS)/opt/vision_sdk
	cp $(vision_sdk_PATH)/linux/src/system/sgxRenderUtils/VertShader.vsh $(LINUX_TARGETFS)/opt/vision_sdk
	cp $(vision_sdk_PATH)/linux/src/system/sgxRenderUtils/FragShader.fsh $(LINUX_TARGETFS)/opt/vision_sdk
	cp $(vision_sdk_PATH)/linux/src/system/sgxRenderUtils/car.pod $(LINUX_TARGETFS)/opt/vision_sdk
