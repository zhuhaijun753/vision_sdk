...
void NullSrcLink_timerCallback(int signo);
void NullSrcLink_CreateTimer(NullSrcLink_Obj *pObj);
void NullSrcLink_DeleteTimer(NullSrcLink_Obj *pObj);
void NullSrcLink_StartTimer(NullSrcLink_Obj *pObj);
void NullSrcLink_StopTimer(NullSrcLink_Obj *pObj);
...
void NullSrcLink_timerCallback(int signo)
{
    ...
    NullSrcLink_Obj *pObj = &gNullSrcLink_obj[0];
...
void NullSrcLink_CreateTimer(NullSrcLink_Obj *pObj)
{
    /* Attache timer handler */
    OSA_attachSignalHandler(SIGALRM, NullSrcLink_timerCallback);

    /* Create Real Time Timer */
    if (timer_create(CLOCK_REALTIME, NULL, &pObj->timerid) < 0)
    {
        Vps_printf(" NULLSRC: Could not create Timer\n");
        OSA_assert(FALSE);
    }
}
...
void NullSrcLink_DeleteTimer(NullSrcLink_Obj *pObj)
{
    ...
    /* Disarm Timer */
    timer_settime(pObj->timerid, 0, &value, NULL);

    timer_delete(pObj->timerid);
}
...
void NullSrcLink_StartTimer(NullSrcLink_Obj *pObj)
{
    ...
    value.it_value.tv_nsec = pObj->createArgs.timerPeriodMilliSecs * 1000000;
    value.it_interval.tv_nsec = pObj->createArgs.timerPeriodMilliSecs * 1000000;
                           ...
                           const System_Buffer *pBuffer)
{
       ...
       ((Int32)TRUE == feof(pObj->fpIndexFile[channelId])))
    {
            ...
            == NULLSRC_LINK_FILEREAD_RUN_TIME)
        {
            ...
            if ((Int32)TRUE == feof(pObj->fpIndexFile[channelId]))
            {
