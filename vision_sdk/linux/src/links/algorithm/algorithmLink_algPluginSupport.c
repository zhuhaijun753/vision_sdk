                                   ...
                                   const AlgorithmLink_FuncTable *pPluginFuncs)
{
    ...
    OSA_assert(algId < ALGORITHM_LINK_ALG_MAXNUM);
                ...
                const AlgorithmLink_OutputQueueInfo *pOutputQInfo)
{
        ...
        queId++)
    {
        ...
        {
            status = OSA_queCreate
                                (&(pObj->emptyOutputQ[queId][chanId].queHandle),
                                ALGORITHM_LINK_MAX_QUEUELENGTH);//, Suriya debug
                                //&(pObj->emptyOutputQ[queId][chanId].queMem),
                                //OSA_QUE_FLAG_NO_BLOCK_QUE);
                                         ...
                                         System_Buffer *pSystemBuffer)
{
    ...
    OSA_assert(outputQId < pObj->numOutputQUsed);

    chId = pSystemBuffer->chNum;

    status = OSA_quePut(&(pObj->emptyOutputQ[outputQId][chId].queHandle),
                          (Int32) pSystemBuffer,
                          OSA_TIMEOUT_NONE);
                                         ...
                                         System_Buffer ** pSystemBuffer)
{
    ...
    OSA_assert(outputQId < pObj->numOutputQUsed);
    OSA_assert(channelId < pObj->outputQInfo[outputQId].queInfo.numCh);
    ...
    status = OSA_queGet(&(pObj->emptyOutputQ[outputQId][channelId].queHandle),
                          (Int32 *) pSystemBuffer,
                          OSA_TIMEOUT_NONE);
                                        ...
                                        System_Buffer *pSystemBuffer)
{
    ...
    status = OSA_quePut(&(pObj->fullOutputQ[outputQId].queHandle),
                          (Int32) pSystemBuffer,
                          OSA_TIMEOUT_NONE);
                                       ...
                                       const Bool        *pBufDropFlag)
{
    ...
    OSA_assert(inputQId < pObj->numInputQUsed);
                        ...
                        AlgorithmLink_BufferFreeStatus  *pBufferFreeStatusBase)
{
    ...
    OSA_mutexCreate(&mutexHndl);

    OSA_mutexLock(&mutexHndl);
...
UInt32 AlgorithmLink_queGetQueuedCount(void *ptr, UInt32 outputQId, UInt32 chId)
{
    ...
    OSA_assert(outputQId < pObj->numOutputQUsed);
    OSA_assert(chId < pObj->outputQInfo[outputQId].queInfo.numCh);
    
    return OSA_queGetQueuedCount(&(pObj->emptyOutputQ[outputQId][chId].queHandle));
}
