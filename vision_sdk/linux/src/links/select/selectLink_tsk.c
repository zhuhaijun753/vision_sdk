 ...
 *
 * \version 0.0 (Nov 2013) : [YM] Ported to HLOS
 *
 *******************************************************************************
 */
                           ...
                           const SelectLink_CreateParams * pPrm)
{
    ...
    OSA_assert(pObj->createArgs.numOutQue <= SELECT_LINK_MAX_OUT_QUE);
    ...
    {
        status = OSA_bufCreate(&pObj->outFrameQue[outQueId], FALSE, FALSE);
        OSA_assert(status == SYSTEM_LINK_STATUS_SOK);
...
Int32 SelectLink_drvProcessData(SelectLink_Obj * pObj)
{
    ...
    if (inBufList.numBuf)
    {
        for (bufId = 0; bufId < inBufList.numBuf; bufId++)
        {
            ...
            OSA_assert(pBuf->chNum < SYSTEM_MAX_CH_PER_OUT_QUE);
            ...
            else
            {
                ...
                OSA_assert(pFrameInfo != NULL);
                flags = pFrameInfo->chInfo.flags;

                /* if rt params are updated copy to local rt params as well */
                if(System_Link_Ch_Info_Get_Flag_Is_Rt_Prm_Update(flags) == (Bool)TRUE)
                {
                            ...
                            System_BufferList * pBufList)
{
    ...
    OSA_assert(queId < (UInt32)SELECT_LINK_MAX_OUT_QUE);
...
Int32 SelectLink_drvDelete(SelectLink_Obj * pObj)
{
    ...
    for(outQueId = 0U; outQueId < pObj->createArgs.numOutQue; outQueId++)
    {
        status = OSA_bufDelete(&pObj->outFrameQue[outQueId]);
        OSA_assert(status == SYSTEM_LINK_STATUS_SOK);
    }
...
Void SelectLink_tskMain(struct OSA_TskHndl * pTsk, OSA_MsgHndl * pMsg, UInt32 curState)
{
    ...
    switch (cmd)
    {
        case SYSTEM_CMD_CREATE:
            ...
            {
                status = SelectLink_drvCreate(pObj, OSA_msgGetPrm(pMsg));
                if(status==SYSTEM_LINK_STATUS_SOK)
                {
        ...
        case SELECT_LINK_CMD_SET_OUT_QUE_CH_INFO:
            if(pObj->state==SYSTEM_LINK_STATE_RUNNING)
            {
                SelectLink_drvSetOutQueChInfo( pObj,
                    (SelectLink_OutQueChInfo*)OSA_msgGetPrm(pMsg));
            }
            OSA_tskAckOrFreeMsg(pMsg, status);
            break;
 ...
 */
 Int32 SelectLink_deInit(void)
 {
    ...
    {
        OSA_tskDelete(&gSelectLink_obj[selectId].tsk);
    }
    return SYSTEM_LINK_STATUS_SOK;
 }
