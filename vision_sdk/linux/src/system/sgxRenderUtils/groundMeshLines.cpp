...
/*VBOs*/
GLuint groundmesh_vbo[1];
GLuint groundmesh_index_vbo[1];
GLuint freetiles_vbo[1];
GLuint freetiles_index_vbo[1];
GLuint carbox_vbo[1];
GLuint carbox_index_vbo[2];

/*Global variables*/
UInt32 numMeshLinesGlobal;
...
void groundMeshLines_draw(Pose3D_f *systemPose_ewInPtr, ObjectBox *boxesInPtr, UInt8 bTileFree[], int viewID)
{
	...
	if (viewID == 0)
	{
		...
		mMVP_gml = mProjection * mView * scale *mModel_box ;
	}
	else
	{
	...
	glUniformMatrix4fv(points_mvMatrixOffsetLoc, 1, GL_FALSE, &mMVP_gml[0][0]);
	...
	if (numFreeTiles>0)
	{
		glUniform4f(colorLoc, FREE_SPACE_COLOR_R, FREE_SPACE_COLOR_G, FREE_SPACE_COLOR_B, 0.3f); //color
	...
	if (numMeshLinesGlobal>0)
	{
		if (viewID==0)
			...
			glLineWidth(2.0f);
		glUniform4f(colorLoc, MESH_COLOR_R, MESH_COLOR_G, MESH_COLOR_B, 1.0f);
	...
	if (viewID==1)
	{
		...
		glUniform4f(colorLoc, SFM_DISP_CAR_BOX_GRAY_LEVEL, SFM_DISP_CAR_BOX_GRAY_LEVEL, SFM_DISP_CAR_BOX_GRAY_LEVEL, 1.0f);
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, carbox_index_vbo[0]);
		glVertexAttribPointer(vertexAttribPosition, 3, GL_FLOAT, GL_FALSE, 0, 0);
		glEnableVertexAttribArray(vertexAttribPosition);
