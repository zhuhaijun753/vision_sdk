...
// POD scene files
const char c_szSceneFile[]                      = "car.pod";
...
//                PVRShellSet(prefExitMessage, "ERROR: Couldn't load the .pod file\n");
        ...
        */
        ...
        /*
                Initialize Print3D
        */
        //bool bRotate = false;//PVRShellGet(prefIsRotated) && PVRShellGet(prefFullScreen);

        if(0)//m_Print3D.SetTextures(0,PVRShellGet(prefWidth),PVRShellGet(prefHeight), bRotate) != PVR_SUCCESS)
        {
...
bool RenderScene(float carScale)
{
        ...
        */
        ...
        for (unsigned int i = 0; i < m_Scene.nNumMeshNode; ++i)
        {
        	...
        	//	mView_glm = glm::make_mat4(mView.f);
        		mWorld_glm = glm::make_mat4(mWorld.f);
        		
			    // VAV sept 30
			    //mView_car = glm::scale(mView, glm::vec3(60.0f));
        		mView_car = glm::scale(mView, glm::vec3(carScale));
        		...
        		PVRTMat4 mView_pvr(&mView[0][0]);
        		PVRTMat4 mProjection_pvr(&mProjection[0][0]);
        		mModelView = mView_pvr * mWorld;
        		mMVP = mProjection_pvr * mModelView;
        		glUniformMatrix4fv(m_ShaderProgram.uiMVPMatrixLoc, 1, GL_FALSE, mMVP.f);


            //    glUniformMatrix4fv(m_ShaderProgram.uiMVPMatrixLoc, 1, GL_FALSE, &mMVP_car[0][0]);
#endif
...
void car_draw_vbo(gl_state *car_gl, int viewid, float carScale )
{
    RenderScene(carScale);
}
