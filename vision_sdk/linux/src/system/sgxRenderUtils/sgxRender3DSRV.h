...
#define POINTS_WIDTH (540)//(440)
#define POINTS_HEIGHT (540)

#define ENABLE_VBO_DRAW
#define ENABLE_CPP

#define SV3D_STEP_SIZE (5.0f) //in mm
...
/*
This same render file is used for different sgx links/layouts.
This enum defines which layout is to be rendered:
*/ 
typedef enum _sgxRender3DMode
{
  RENDER_MODE_3D_SURROUND_VIEW = 0,
  RENDER_MODE_3D_PERCEPTION = 1
} sgxRender3DMode;
...
typedef struct
{
   ...
   float carScale; /*scale of car model*/

   int inFramesWidth;
   int inFramesHeight;

   /*mode decides which setup is rendered*/
   sgxRender3DMode mode;
