...
{
    billboard_program = SgxRender3DSRV_createProgram(billboard_vshader, billboard_fshader);
...
*
=======================================================================*/
void billboard_draw(float boxes[][24], Point3D_f base[],  UInt32 numBoxes,
			Pose3D_f *pose_ew, Pose3D_f *poses_cw, GLuint *texYuv)
{
	...
	scale = glm::scale(glm::mat4(1.0f), glm::vec3(1/SV3D_STEP_SIZE, 1/SV3D_STEP_SIZE, 1/SV3D_STEP_SIZE));
	...
	for (ii = 0; ii < numBoxes; ii++)
	{
		...
		Matrix3D_f R_ev;
		R_ev.xx = (Flouble)mView[0][0]; R_ev.xy = (Flouble)mView[0][1]; R_ev.xz = (Flouble)mView[0][2];
		R_ev.yx = (Flouble)mView[1][0]; R_ev.yy = (Flouble)mView[1][1]; R_ev.yz = (Flouble)mView[1][2];
		R_ev.zx = (Flouble)mView[2][0]; R_ev.zy = (Flouble)mView[2][1]; R_ev.zz = (Flouble)mView[2][2];
		...
		mMVP_billboard = mProjection * mView * scale * track * mMV_billboard;
...
*
=======================================================================*/
void distance_info_draw(Point3D_f base[],  UInt32 numBoxes,
							Pose3D_f *pose, float dists[])
{
	...
	glm::mat4 M_pvm = mProjection * mView * scale * mModel;
