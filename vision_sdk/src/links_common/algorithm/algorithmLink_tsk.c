...
Void AlgorithmLink_tskMain(struct Utils_TskHndl_t *pTsk, Utils_MsgHndl *pMsg)
{
    ...
    switch (cmd)
    {
        ...
        case SYSTEM_CMD_NEW_DATA:
            ...
            {
                if(gAlgorithmLinkFuncTable[pObj->algId].
                   AlgorithmLink_AlgPluginProcess != NULL)
                {
                    #if defined (BUILD_ARP32)
                    Utils_idleEnableEveDMA();
                    #endif

                    ffiMode = gAlgorithmLinkFFIMode[pObj->algId];

                    if(BSP_SAFETY_OSAL_MODE_QM == ffiMode)
                    {
                        cookie = Hwi_disable();
                        BspSafetyOsal_setSafetyMode(BSP_SAFETY_OSAL_MODE_QM);
                    ...
                    status =
                        (*(gAlgorithmLinkFuncTable[pObj->algId].
                        AlgorithmLink_AlgPluginProcess))((void *) pObj);
...
#endif
              if(gAlgorithmLinkFuncTable[pObj->algId].
                 AlgorithmLink_AlgPluginControl != NULL)
              {
               status =
               (*(gAlgorithmLinkFuncTable[pObj->algId].
                AlgorithmLink_AlgPluginControl))
                    ((void *) pObj,
                     (void *) pControlParams
                    );
              }
        ...
        case ALGORITHM_LINK_CMD_GET_ALGID:
            *((UInt32*)Utils_msgGetPrm(pMsg)) = pObj->algId;
            Utils_tskAckOrFreeMsg(pMsg, status);
            break;
        ...
        case SYSTEM_CMD_DELETE:
            ...
            if(pObj->state==SYSTEM_LINK_STATE_STOPPED)
            {
                ...
                if(gAlgorithmLinkFuncTable[pObj->algId].
                   AlgorithmLink_AlgPluginDelete != NULL)
                {
                    status =
                        (*(gAlgorithmLinkFuncTable[pObj->algId].
                        AlgorithmLink_AlgPluginDelete)) ((void *) pObj);
                }
