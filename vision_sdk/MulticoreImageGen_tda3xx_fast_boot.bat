...
REM In order to disable CRC comment below line
REM Also comment LE image generation in image_gen block and uncomment BE image generation
set calculate_crc=1
...
"%Tools_path%\multicore_image_generator\v2\MulticoreImageGen.exe" LE %Dev_ID% %Out_Path%\AppImage_UcEarly_LE %IPU1_CPU0% %App_IPU1_CPU0_RPRC% %IPU1_CPU1% %App_IPU1_CPU1_RPRC%
"%Tools_path%\multicore_image_generator\v2\MulticoreImageGen.exe" LE %Dev_ID% %Out_Path%\AppImage_UcLate_LE %DSP1_CPU% %App_DSP1_RPRC% %DSP2_CPU% %App_DSP2_RPRC% %EVE1_CPU% %App_EVE1_RPRC% %IPU1_CPU_SMP% %App_IPU1_RPRC%
)

if defined calculate_crc (
"%Tools_CRC_path%\crc_multicore_image.exe" %Out_Path%\AppImage_UcEarly_LE %Out_Path%\AppImage_UcEarly_BE
"%Tools_CRC_path%\crc_multicore_image.exe" %Out_Path%\AppImage_UcLate_LE %Out_Path%\AppImage_UcLate_BE

del %Out_Path%\AppImage_UcEarly_LE
del %Out_Path%\AppImage_UcLate_LE
)
