...
void Usecase::genConnectLinks(ostream& fp, ostream& fph) {
    ...
    for (int i = 0; i < exec_seq.size(); i++) {
        ...
        for (int j = 0; j < (outLinkPtr->size()); j++) {
            ...
            /* For the Synthesis AlgoPlugin layer
                PhotoSynthesis is expected on input queue 2 and
                GAlign is expected on input queue 1,
                so hard coding here. */
            if ("Alg_Synthesis" == B->getName())
            {
                if ("Alg_PhotoAlign" == A->getName())
                {
                    num = 2;
                }
                if ("Alg_GeoAlign" == A->getName())
                {
                    num = 1;
                }
            }
