 ...
 *******************************************************************************
 *
 * \file safeFrameCopyAlgoLocalDma.c
 *
 * \brief Algorithm for Alg_SafeFrameCopy on DSP/EVE using Local EDMA
 *
...
typedef struct
{
    Alg_SafeFrameCopy_Obj     safeFrameCopyObj;
                        ...
                        Alg_SafeFrameCopyCreateParams *pCreateParams)
{
    ...
    UInt32 paramPhyAddr;
    Alg_SafeFrameCopyDma_Obj * pAlgHandle;

    pAlgHandle = (Alg_SafeFrameCopyDma_Obj *) malloc(sizeof(Alg_SafeFrameCopyDma_Obj));

    UTILS_assert(pAlgHandle != NULL);

    pAlgHandle->safeFrameCopyObj.maxHeight   = pCreateParams->maxHeight;
    pAlgHandle->safeFrameCopyObj.maxWidth    = pCreateParams->maxWidth;
    ...
    {
        Vps_printf(" ALG_SAFEFRAMECOPY: DMA: Allocated CH (TCC) = %d (%d)\n",
                        pAlgHandle->edmaChId,
                        pAlgHandle->tccId);
    ...
    {
        Vps_printf(" ALG_SAFEFRAMECOPY: DMA: ERROR in EDMA CH allocation\n");
    }
    ...
    return &pAlgHandle->safeFrameCopyObj;
}
                          ...
                          )
{
    ...
    pAlgHandle = (Alg_SafeFrameCopyDma_Obj *)algHandle;
