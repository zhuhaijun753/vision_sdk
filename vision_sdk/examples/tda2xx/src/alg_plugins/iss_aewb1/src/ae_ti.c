        ...
        const IALG_Params * algParams)
{
    ...
    {
        h->prevAE[i].exposureTime = (XDAS_Int32)1000;
        h->prevAE[i].apertureLevel = (XDAS_Int32)0;
        h->prevAE[i].sensorGain = (XDAS_Int32)1000;
        h->prevAE[i].ipipeGain = (XDAS_Int32)128;
    }
...
//    unsigned int temp;
    ...
    int reset = ((h->avgY == -1) || (inArgs->curAe.exposureTime % h->exposureTimeStepSize != 0));
      ...
      cT = h->prevAE[chId].exposureTime;
      cAG = h->prevAE[chId].sensorGain;
      cA = h->prevAE[chId].apertureLevel;
      cDG = h->prevAE[chId].ipipeGain;
...
*/
    ...
    if (curY == 0)
    {
      ...
      if(numAEChannels == IAE_MIN_CHANNELS)
      {
      /*DISABLE_HISTORY in case of 4 channel SRV*/
      /*Changes are needed to maintain seperate histories for all the channels*/
      /*Till this is done, history is disabled. Downside is that AE tranistions will not be as smooth  */
   ...
   }//numAEChannels == IAE_MIN_CHANNELS
    ...
    h->prevAE[chId].exposureTime = outArgs->nextAe.exposureTime = nT;
    h->prevAE[chId].apertureLevel =  outArgs->nextAe.apertureLevel = nA;
    h->prevAE[chId].sensorGain = outArgs->nextAe.sensorGain = nAG;
    h->prevAE[chId].ipipeGain = outArgs->nextAe.ipipeGain = nDG;
